...
function DesktopNavbar() {
  return (
    <div
      className='fixed top-0 left-1/2 z-40 h-fit w-1/2 -translate-x-1/2 rounded-lg shadow-lg'
      style={{
        background:
          'linear-gradient(135deg, rgba(255, 255, 255, 0.1), rgba(128, 128, 128, 0.2))',
        backdropFilter: 'blur(10px)',
      }}
    >
      <div className='hidden md:block'>
        <nav className='container flex items-center justify-between gap-x-4 px-10 py-2'>
          <Logo />
                ...
                item.label === 'About' ? (
                  <DropdownMenu key={item.label}>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant={'outline'}
                        className='w-full gap-2 justify-start text-lg text-muted-foreground hover:text-foreground'
                      >
                        {item.icon}
                      </Button>
                    </DropdownMenuTrigger>
